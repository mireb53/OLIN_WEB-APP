<x-layouts.auth :title="'Reset Password - OLIN'">
  <div class="container max-w-md mx-auto p-6">
    ...
    <form method="POST" action="{{ route('password.update') }}" class="space-y-4">
      ...
      <input type="email" name="email" value="{{ $email ?? old('email') }}" placeholder="Email" class="w-full border rounded px-3 py-2" required>
      @error('email')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
      <input type="password" name="password" placeholder="New Password" class="w-full border rounded px-3 py-2" required>
